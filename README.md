...
```
1. 在项目目录运行 ahahooh init
2. 启动 Claude Code（正常使用）
3. Hook 自动记录每次 Write/Edit/Bash 操作
4. Claude 在关键决策点自动调用 save_conversation
5. Claude 在形成计划时自动调用 save_plan
6. 会话结束时，Stop hook 自动重建索引
7. 下次启动 Claude Code，输入 /resume 恢复上下文
```
